...
export class DashboardService {
  ...
  async get() {
    const [confirmedReservations, brokerCount, unitCounts, todayCount, recent] =
      await Promise.all([
    ...
    const totalSales = confirmedReservations.reduce(
      (sum, r) => sum + r.unit.price,
      0,
    );
    const totalUnits = unitCounts.reduce((sum, u) => sum + u._count._all, 0);
    const availableUnits =
      unitCounts.find((u) => u.status === 'available')?._count._all ?? 0;
    const occupancy =
      totalUnits > 0
    ...
    const breakdown = unitCounts
      ...
      .map((u) => ({
        ...
        units: u._count._all,
        pct:
          totalUnits > 0 ? Math.round((u._count._all / totalUnits) * 100) : 0,
      }));
